...
static const char* BUFFER_MODE = "BUFFER_MODE";
static const char* STREAM_MODE = "STREAM_MODE";
...
class ConfigTest : public ::testing::Test
{
  protected:
    ...
    {
        unsetenv(SOCKET_ID);
        unsetenv(BUFFER_MODE);
        unsetenv(STREAM_MODE);
...
TEST_F(ConfigTest, Defaults)
{
    ...
    EXPECT_EQ(cfg.bufferModeEnabled, true);
    EXPECT_EQ(cfg.streamModeEnabled, false);
...
TEST_F(ConfigTest, Load)
{
    ...
    setenv(STREAM_MODE, "true", 1);
    setenv(BUFFER_MODE, "true", 1);
    ...
    EXPECT_EQ(cfg.streamModeEnabled, true);
    EXPECT_EQ(cfg.bufferModeEnabled, true);
...
TEST_F(ConfigTest, InvalidBoolean)
{
    ...
    EXPECT_THROW(Config(), std::invalid_argument);
    setenv(FLUSH_FULL, "true", 1);
    setenv(BUFFER_MODE, "invalid", 1);
    EXPECT_THROW(Config(), std::invalid_argument);
    setenv(FLUSH_FULL, "true", 1);
    setenv(BUFFER_MODE, "false", 1);
    setenv(STREAM_MODE, "invalid", 1);
    EXPECT_THROW(Config(), std::invalid_argument);
    setenv(STREAM_MODE, "true", 1);
    EXPECT_NO_THROW(Config());
}

TEST_F(ConfigTest, NoActiveModes)
{
    setenv(BUFFER_MODE, "false", 1);
    setenv(STREAM_MODE, "false", 1);
    EXPECT_THROW(Config(), std::invalid_argument);
    setenv(BUFFER_MODE, "true", 1);
    setenv(STREAM_MODE, "false", 1);
    EXPECT_NO_THROW(Config());
...
TEST_F(ConfigTest, InvalidStreamModeConfig)
{
    ...
    setenv(FLUSH_FULL, tooLong.c_str(), 1);
    ASSERT_THROW(Config(), std::invalid_argument);
}
