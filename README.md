...
by the rsyslog imuxsock module). In this mode, logs are persisted as soon as
collected. It fits in the Redfish EventService architecture as well.
...
The Host Logger service in the stream mode forwards the byte stream into rsyslog
via the imuxsock module. The log is persisted via the omfile module as soon as
collected. It makes Host Logger leverage exsisting tools (rsyslog and logrotate).
